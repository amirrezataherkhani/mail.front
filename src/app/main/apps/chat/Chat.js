...
function Chat(props) {
    ...
    const singleChatQuery = SingleChatUseQuery();
    const [chatType, setChatType] = useState("");
    const [user, setUser] = useState({});
    const getUserIdQuery = GetUserIdUseQuery();
    const classes = useStyles(props);
    const chatRef = useRef(null);
    const [messageText, setMessageText] = useState("");

    const getChatData = (chatId) => {
        singleChatQuery.mutate(
            { chatId },
            {
                onSuccess: (result) => {
                    console.log(result);
                    setChatType(result.data.type);
                },
            }
        );
    };
    ...
    function onMessageSubmit(ev) {
        ...
        dispatch(Actions.sendMessage(messageText, chat.chatId, user.id)).then(
            () => {
                setMessageText("");
            }
        );
                    ...
                    >
                        {chat && chat.dialog.length > 0 ? (
                            <div className="flex flex-col pt-16 pl-56 pr-16 pb-40">
                                {[...chat.dialog].reverse().map((item, i) => {
                                    const contact =
                                        item.who === user.id
                                            ? user
                                            : contacts.find(
                                                  (_contact) =>
                                                      _contact.id === item.who
                                              );
                                    ...
                                    return (
                                        ...
                                        >
                                            {/* {shouldShowContactAvatar(
                                                ...
                                                <Avatar
                                                    className="avatar absolute left-0 m-0 -ml-32"
                                                    src={contact.avatar}
                                                />
                                            )} */}
